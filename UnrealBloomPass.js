...
class UnrealBloomPass extends Pass {
	...
	constructor( resolution, strength = 1, radius, threshold ) {
		...
		// render targets
		const pars = { // Added for transparent background fix
			minFilter: LinearFilter,
			magFilter: LinearFilter,
			format: RGBAFormat,
			type: HalfFloatType // Keep HalfFloatType for performance
		};
		...
		for ( let i = 0; i < this.nMips; i ++ ) {
			...
			this.separableBlurMaterials[ i ].uniforms[ 'invSize' ].value = new Vector2( 1 / resx, 1 / resy ); // Changed from texSize to invSize to match original code
